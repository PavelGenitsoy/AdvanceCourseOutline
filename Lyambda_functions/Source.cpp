...
int main(int argc, const char* argv[]) {
	...
	[](bool value) -> int { // явно указываем типа для вовзращаемого значения, когда такая спорная ситуация
		if (value) {
			return 10.0f;
		}
		else {
	...
	[] { // можно не указывать скобки, где находятся аргументы при вызове лямбда, но лучше указывать (так понятнее будет)
